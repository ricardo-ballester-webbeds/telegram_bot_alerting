...
if response.status_code == 200 and response.json().get("ok"):
    ...
    asyncio.run_coroutine_threadsafe(
        application.bot.send_message(chat_id=CHANNEL_ID, text='Webhook configured successfully! 🚀\rBot is still not running.'),
        loop
    )
else:
    print(f"❌ Error configuring webhook: {response.text}")
    asyncio.run_coroutine_threadsafe(
        application.bot.send_message(chat_id=CHANNEL_ID, text='Webhook configuration failed. Please check LocalTunnel and try again. ❌'),
        loop
    )
    lt_process.terminate()
    exit(1)
